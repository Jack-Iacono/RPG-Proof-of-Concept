...
for(i = 0; i < instance_number(obj_enemy_battle); i++){
	...
	for(j = 0; j < array_length(instance_find(obj_enemy_battle, i).status_array); j++){
		print_text_box(enemy_stats_x + (j * 70), enemy_stats_y + 120, instance_find(obj_enemy_battle, j).status_array[j].color, c_black, instance_find(obj_enemy_battle, j).status_array[j].abbreviation, seperation, width, font_stats);
	}
